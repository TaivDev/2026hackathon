...
| `getAreaRevenue(area, fullSchedule, ads, decayRate)` | Return the total revenue for the given area using the full schedule and decay rate. Decay is based on the full schedule globally — not just this area. Return `0` if the area's `areaId` is not a key in the schedule. |
