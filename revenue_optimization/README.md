...
| Method | Description |
|--------|-------------|
| `getNextAvailableStartTime(areaSchedule)` | Return the earliest next available start time in the given area schedule. |
| `getValidCandidatePlacements(ads, areas, schedule)` | Return all currently valid candidate placements as `{ adId, areaId, startTime }` objects. |
| `selectBestNextPlacement(ads, areas, schedule, decayRate)` | Return the single best next valid placement based on the scoring rules, or `null` if no valid placement remains. |
| `buildOptimizedSchedule(ads, areas, decayRate)` | Build and return a complete optimized schedule across all areas while respecting all constraints. |
